...
export default function StaffSection({ selectedTeamId }) {
  ...
  const isLoading = staffLoading || teamLoading
  ...
  return (
    <div>
      ...
      <div className="flex gap-6 mt-5">
        ...
        <div className="w-64 flex-shrink-0 self-start sticky top-5 hidden lg:block">
          ...
          <div className="space-y-3">
            {staff.map(member => {
              if (expandedId === member.id) {
                const edit = localEdits[member.id]
                if (!edit) return null
                return (
                  <div key={member.id} className="p-4 rounded-xl border-2 border-pink-300 bg-white shadow-sm">
                    ...
                    <div className="flex items-center gap-2 mb-3 flex-wrap">
                      ...
                      <input
                        type="text"
                        value={edit.name}
                        onChange={(e) => updateLocal(member.id, 'name', e.target.value)}
                        placeholder="Name"
                      ...
                      <input
                        type="email"
                        value={edit.email}
                        onChange={(e) => updateLocal(member.id, 'email', e.target.value)}
                        placeholder="Email"
                      ...
                      <div className="flex items-center gap-1">
                        <input type="number" value={edit.contracted_hours} onChange={(e) => updateLocal(member.id, 'contracted_hours', e.target.value)} placeholder="Hrs" min="0" className="w-14 px-2 py-2 border border-gray-300 rounded-lg text-sm text-center text-gray-900 focus:ring-2 focus:ring-pink-500 focus:border-transparent" />
                        <span className="text-[10px] text-gray-400">h/wk</span>
                      </div>
                      <div className="flex items-center gap-1">
                        <input type="number" value={edit.max_hours} onChange={(e) => updateLocal(member.id, 'max_hours', e.target.value)} placeholder="Max" min="0" className="w-14 px-2 py-2 border border-gray-300 rounded-lg text-sm text-center text-gray-900 focus:ring-2 focus:ring-pink-500 focus:border-transparent" />
                        <span className="text-[10px] text-gray-400">max</span>
                      </div>
                      <div className="flex items-center gap-1">
                        <span className="text-[10px] text-gray-400">£</span>
                        <input type="number" value={edit.hourly_rate} onChange={(e) => updateLocal(member.id, 'hourly_rate', e.target.value)} placeholder="Rate" step="0.01" min="0" className="w-14 px-2 py-2 border border-gray-300 rounded-lg text-sm text-center text-gray-900 focus:ring-2 focus:ring-pink-500 focus:border-transparent" />
                        <span className="text-[10px] text-gray-400">/hr</span>
                      </div>
                      <div className="flex items-center gap-1.5">
                        <span className="text-xs text-gray-500 font-medium">Keyholder</span>
                        <button type="button" onClick={() => updateLocal(member.id, 'keyholder', !edit.keyholder)} className={`relative w-10 h-[22px] rounded-full transition-colors flex-shrink-0 ${edit.keyholder ? 'bg-orange-400' : 'bg-gray-200'}`}>
                      ...
                      <button onClick={() => cancelEdit(member.id)} className="p-1.5 text-gray-400 hover:text-gray-600 transition-colors flex-shrink-0" title="Close">
                    ...
                    {shiftLengths.length > 0 && (
                      <div className="flex items-center gap-1.5 mb-2">
                        ...
                        {shiftLengths.map((len, li) => {
                          ...
                          return (
                            <button key={len} onClick={() => togglePrefLength(member.id, len)} className="px-2 py-0.5 rounded text-[10px] font-bold border transition-all" style={{ background: active ? c.bg : '#F9FAFB', borderColor: active ? `${c.border}50` : '#E5E7EB', color: active ? c.text : '#9CA3AF' }}>
                              {len}h
                            </button>
                          )
                        })}
                    ...
                    {Object.keys(templates).length > 0 && (
                      <StaffAvailabilityGrid
                        ...
                        onChange={(grid) => updateLocal(member.id, 'availability_grid', grid)}
                      />
                    )}
                    ...
                    <div className="mt-3 flex items-center justify-between gap-2 flex-wrap pt-3 border-t border-gray-100">
                      ...
                      <div className="flex items-center gap-2">
                        <button onClick={() => cancelEdit(member.id)} className="px-3 py-1.5 rounded-lg border border-gray-300 text-xs font-medium text-gray-600 hover:bg-gray-50 transition-colors">
                          Cancel
                        </button>
                        <button
                          onClick={() => saveAndCollapse(member.id)}
                        ...
                        >
                          {updateStaffMut.isPending ? 'Saving...' : 'Save'}
                        </button>
                      </div>
                    </div>
                  </div>
            ...
            {newStaffId && expandedId === newStaffId && (() => {
              const edit = localEdits[newStaffId]
              if (!edit) return null
              return (
                <div className="p-4 rounded-xl border-2 border-pink-300 bg-white shadow-sm">
                  {/* Row 1: Inline inputs */}
                  <div className="flex items-center gap-2 mb-3 flex-wrap">
                    <div className="w-9 h-9 rounded-lg bg-gradient-to-br from-pink-50 to-purple-50 flex items-center justify-center text-pink-600 font-bold text-sm flex-shrink-0">
                      {(edit.name || '?')[0].toUpperCase()}
                    ...
                    <input
                      type="text"
                      value={edit.name}
                      onChange={(e) => updateLocal(newStaffId, 'name', e.target.value)}
                      placeholder="Name"
                      autoFocus
                      className="flex-1 min-w-[100px] px-3 py-2 border border-gray-300 rounded-lg text-sm font-semibold text-gray-900 focus:ring-2 focus:ring-pink-500 focus:border-transparent"
                    />
                    <input
                      type="email"
                      value={edit.email}
                      onChange={(e) => updateLocal(newStaffId, 'email', e.target.value)}
                      placeholder="Email"
                      className="min-w-[140px] px-3 py-2 border border-gray-300 rounded-lg text-sm text-gray-900 focus:ring-2 focus:ring-pink-500 focus:border-transparent"
                    />
                    <div className="flex items-center gap-1">
                      <input type="number" value={edit.contracted_hours} onChange={(e) => updateLocal(newStaffId, 'contracted_hours', e.target.value)} placeholder="Hrs" min="0" className="w-14 px-2 py-2 border border-gray-300 rounded-lg text-sm text-center text-gray-900 focus:ring-2 focus:ring-pink-500 focus:border-transparent" />
                      <span className="text-[10px] text-gray-400">h/wk</span>
                    </div>
                    <div className="flex items-center gap-1">
                      <input type="number" value={edit.max_hours} onChange={(e) => updateLocal(newStaffId, 'max_hours', e.target.value)} placeholder="Max" min="0" className="w-14 px-2 py-2 border border-gray-300 rounded-lg text-sm text-center text-gray-900 focus:ring-2 focus:ring-pink-500 focus:border-transparent" />
                      <span className="text-[10px] text-gray-400">max</span>
                    </div>
                    <div className="flex items-center gap-1">
                      <span className="text-[10px] text-gray-400">£</span>
                      <input type="number" value={edit.hourly_rate} onChange={(e) => updateLocal(newStaffId, 'hourly_rate', e.target.value)} placeholder="Rate" step="0.01" min="0" className="w-14 px-2 py-2 border border-gray-300 rounded-lg text-sm text-center text-gray-900 focus:ring-2 focus:ring-pink-500 focus:border-transparent" />
                      <span className="text-[10px] text-gray-400">/hr</span>
                    </div>
                    <div className="flex items-center gap-1.5">
                      <span className="text-xs text-gray-500 font-medium">Keyholder</span>
                      <button type="button" onClick={() => updateLocal(newStaffId, 'keyholder', !edit.keyholder)} className={`relative w-10 h-[22px] rounded-full transition-colors flex-shrink-0 ${edit.keyholder ? 'bg-orange-400' : 'bg-gray-200'}`}>
                        <div className={`absolute w-[18px] h-[18px] bg-white rounded-full top-[2px] shadow-sm transition-all ${edit.keyholder ? 'left-[20px]' : 'left-[2px]'}`} />
                      </button>
                    </div>
                    <button onClick={() => cancelEdit(newStaffId)} className="p-1.5 text-gray-400 hover:text-gray-600 transition-colors flex-shrink-0" title="Cancel">
                      <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
                    </button>
                  </div>
                  ...
                  {shiftLengths.length > 0 && (
                    <div className="flex items-center gap-1.5 mb-2">
                      ...
                      {shiftLengths.map((len, li) => {
                        ...
                        const c = PALETTE[li % PALETTE.length]
                        return (
                          <button key={len} onClick={() => togglePrefLength(newStaffId, len)} className="px-2 py-0.5 rounded text-[10px] font-bold border transition-all" style={{ background: active ? c.bg : '#F9FAFB', borderColor: active ? `${c.border}50` : '#E5E7EB', color: active ? c.text : '#9CA3AF' }}>
                            {len}h
                          </button>
                        )
                      })}
                  ...
                  {Object.keys(templates).length > 0 && (
                    <StaffAvailabilityGrid
                      ...
                      preferredLengths={edit.preferred_lengths}
                      availabilityGrid={edit.availability_grid}
                      onChange={(grid) => updateLocal(newStaffId, 'availability_grid', grid)}
                    />
                  )}

                  {/* Footer: save/cancel */}
                  <div className="mt-3 flex items-center justify-end gap-2 pt-3 border-t border-gray-100">
                    <button onClick={() => cancelEdit(newStaffId)} className="px-3 py-1.5 rounded-lg border border-gray-300 text-xs font-medium text-gray-600 hover:bg-gray-50 transition-colors">
                      Cancel
                    </button>
                    <button
                      onClick={() => saveAndCollapse(newStaffId)}
                      disabled={addStaffMut.isPending}
                      className="px-4 py-1.5 rounded-lg text-xs font-semibold text-white transition-all hover:shadow-md disabled:opacity-50"
                      style={{ background: '#FF1F7D' }}
                    >
                      {addStaffMut.isPending ? 'Adding...' : 'Add Staff'}
                    </button>
                  </div>
                </div>
              )
