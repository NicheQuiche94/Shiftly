      ...
      <main className="max-w-md mx-auto px-6 py-20">
        <div className="bg-white rounded-2xl border-2 border-gray-200 p-8 shadow-sm">
          <div className="text-center mb-8">
            ...
            <h1 className="text-2xl font-bold text-gray-900 mb-2">Payroll Access</h1>
      ...
      <div className="mb-8">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-600">Manage staff pay rates and calculate rota costs</p>
          </div>
          <button
            onClick={() => {
      ...
      {activeTab === 'pay' && (
        <div className="bg-white rounded-2xl border-2 border-gray-200 overflow-hidden">
          <div className="px-6 py-4 border-b border-gray-200 bg-gray-50">
            <h3 className="font-semibold text-gray-900">Staff Pay Information</h3>
            <p className="text-sm text-gray-500">Set hourly rates or annual salaries for each team member</p>
          </div>
          ...
          {loading ? (
            ...
            <div className="text-center py-12">
              <p className="text-gray-500">Select a team to view staff pay information</p>
            </div>
          ) : staffPayroll.length === 0 ? (
            <div className="text-center py-12">
              <p className="text-gray-500">No staff members in this team</p>
            </div>
          ) : (
                    ...
                    <div className="space-y-4">
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <p className="text-sm text-gray-500">{staff.contracted_hours}h/week contracted</p>
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        ...
                        <div className="flex items-end gap-2">
                          <button
                            ...
                            className="px-4 py-2 bg-gradient-to-r from-pink-500 to-pink-600 text-white rounded-lg font-medium hover:shadow-lg transition-all"
                          >
                          ...
                          <div className="text-right">
                            <p className="font-semibold text-gray-900">
                              {staff.pay_type === 'hourly' 
                                ...
                                : `${formatCurrency(staff.annual_salary)}/year`
                              }
                            </p>
                            <p className="text-sm text-gray-500 capitalize">{staff.pay_type}</p>
                          </div>
                        ) : (
                          <div className="text-right">
                            <p className="text-amber-600 font-medium">Not Set</p>
                            <p className="text-sm text-gray-500">Add pay info</p>
                          </div>
                        )}
      ...
      {activeTab === 'costs' && (
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-2xl border-2 border-gray-200 p-6">
            <h3 className="font-semibold text-gray-900 mb-4">Select Rota</h3>
          ...
          {loadingCosts ? (
            ...
            <div className="bg-white rounded-2xl border-2 border-gray-200 overflow-hidden">
              <div className="px-6 py-4 border-b border-gray-200 bg-gray-50 flex items-center justify-between">
                <div>
                  <h3 className="font-semibold text-gray-900">{rotaCosts.rota_name}</h3>
                  <p className="text-sm text-gray-500">
                    {formatDate(rotaCosts.start_date)} - {formatDate(rotaCosts.end_date)} • {rotaCosts.week_count} week{rotaCosts.week_count > 1 ? 's' : ''}
                  </p>
                </div>
                <div className="text-right">
                  <p className="text-2xl font-bold text-gray-900">{formatCurrency(rotaCosts.total_cost)}</p>
                  ...
                  <div key={idx} className="px-6 py-4 flex items-center justify-between">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-gray-500">
                          {staff.totalHours.toFixed(1)}h total
                          {staff.payType === 'hourly' && staff.hourlyRate && ` @ ${formatCurrency(staff.hourlyRate)}/hr`}
                          {staff.payType === 'salary' && staff.annualSalary && ` (${formatCurrency(staff.annualSalary)}/year)`}
                        </p>
                      </div>
                    </div>
                    <div className="text-right">
                      {staff.cost > 0 ? (
                        <p className="font-semibold text-gray-900">{formatCurrency(staff.cost)}</p>
                      ) : (
                        <p className="text-amber-600 text-sm">Pay not set</p>
                      )}
                    </div>
                  </div>
                ))}
              ...
              <div className="px-6 py-4 bg-gray-50 border-t border-gray-200">
                <div className="flex items-center justify-between">
                  ...
                  <p className="text-2xl font-bold text-pink-600">{formatCurrency(rotaCosts.total_cost)}</p>
                </div>
              </div>
            </div>
          ) : selectedRotaId ? (
            <div className="bg-white rounded-2xl border-2 border-gray-200 p-12 text-center">
              <p className="text-gray-500">Failed to load costs. Please try again.</p>
            </div>
          ) : (
            <div className="bg-white rounded-2xl border-2 border-gray-200 p-12 text-center">
              ...
              <p className="text-gray-600 font-medium">Select a rota to view costs</p>
              <p className="text-sm text-gray-500 mt-1">Only approved rotas are shown</p>
            </div>
          )}
        </div>
      )}
